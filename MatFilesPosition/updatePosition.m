function [IDvehicleExit,positionManagement] = updatePosition(IDvehicle, positionManagement, simParams)
...
switch simParams.typeOfScenario
    case {constants.SCENARIO_PPP, constants.SCENARIO_HIGHWAY}  % PPP or Highway
        if ~simParams.linkSUMO
            Xvehicle = mod(Xvehicle + positionManagement.direction.*positionManagement.v*simParams.positionTimeResolution,positionManagement.Xmax);
            positionManagement.XvehicleReal = Xvehicle;
            positionManagement.YvehicleReal = Yvehicle;
        else  % update lacation from sumo
            traci.simulation.step;  % each step equals to the simParams.positionTimeResolution
            teleportOutsideVehicles();  % if vehicles run out of road, move it to the other point of the same lane
        ...
        IDvehicleExit = [];
    case constants.SCENARIO_URBAN       % Urban
